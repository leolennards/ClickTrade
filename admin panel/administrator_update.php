<?php
    ...
    if (isset($_POST['submit'])) {
        ...
        if (!empty($name)) {
            ...
            // Also update the users table if this seller is also a user
            $update_user_name = $conn->prepare("UPDATE `users` SET name = ? WHERE id = (SELECT user_id FROM sellers WHERE id = ?)");
            $update_user_name->execute([$name, $seller_id]);
        ...
        if ($old_pass != $empty_pass) {
            if ($old_pass != $prev_pass) {
                ...
                if ($new_pass != $empty_pass) {
                    ...
                    // Also update the users table if this seller is also a user
                    $update_user_pass = $conn->prepare("UPDATE `users` SET password = ? WHERE id = (SELECT user_id FROM sellers WHERE id = ?)");
                    $update_user_pass->execute([$cpass, $seller_id]);
